export default function GlassCard({ children, className = '', glow = false }) {
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.03] p-6 backdrop-blur-xl transition-all duration-600 hover:border-white/[0.14] hover:bg-white/[0.05] ${glow ? 'shadow-card-glow' : 'shadow-card'} ${className}`}
    >
